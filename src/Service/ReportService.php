...
class ReportService
{
    ...
    public function getSalesForRange(Business $business, \DateTimeImmutable $from, \DateTimeImmutable $to, array $filters = []): array
    {
        $qb = $this->saleRepository->createQueryBuilder('s')
            ...
            ->addSelect('s.status AS saleStatus')
            ->addSelect('s.voidedAt AS voidedAt')
            ->addSelect('s.voidReason AS voidReason')
            ...
            ->addSelect('MAX(acn.id) AS arcaCreditNoteId')
            ->addSelect('MAX(acn.status) AS arcaCreditNoteStatus')
            ->addSelect('MAX(acn.cbteNumero) AS arcaCreditNoteNumber')
            ->addSelect('MAX(acn.cae) AS arcaCreditNoteCae')
            ->addSelect('MAX(acn.issuedAt) AS arcaCreditNoteIssuedAt')
            ...
            ->leftJoin(\App\Entity\ArcaCreditNote::class, 'acn', 'WITH', 'acn.sale = s AND acn.business = :business')
            ...
            ->setParameter('to', $to)
            ->setParameter('statuses', [\App\Entity\Sale::STATUS_CONFIRMED, \App\Entity\Sale::STATUS_VOIDED])
            ->setParameter('defaultCustomer', 'Consumidor final')
            ->groupBy('s.id, u.email, c.name, s.total, s.createdAt, s.status, s.voidedAt, s.voidReason')
